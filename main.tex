...
\usepackage{hyperref}
\usepackage[portuguese]{babel} % Garante hifenização e nomes em PT-BR
...
A sensação de infinitude é obtida pela repetição de segmentos (\texttt{NUM\_SEGMENTOS = 80}) posicionados à frente da câmera. O traçado recebe um \textit{offset} senoidal, simulando curvas largas. Cada bloco também desenha arquibancadas em ambos os lados, texturizadas com madeira e preenchidas por “bonecos” que saltam em ritmos pseudo-aleatórios.
...
\begin{itemize}
    ...
    \item Carro fiel McLaren M23/1976, com piloto;
...
Apesar do êxito geral, é importante mencionar que o modelo final do carro não ficou 100\% fiel ao modelo de exemplo utilizado como referência. Isso se deveu à decisão de priorizar o desenvolvimento do ambiente, do sistema de partículas e da implementação das mecânicas de interação e ações na cena, garantindo uma experiência de simulação mais fluida e reativa em detrimento de um detalhamento geométrico exaustivo do veículo.
